...
public class UIManager : MonoBehaviour
{
    ...
    [Space(10)]
    [SerializeField] Text soundBtnText;
    [SerializeField] Text vibroBtnText;
    ...
    {
        SoundManager.Instance.SetMute(out bool IsMute);
        string color = IsMute ? "red" : "green";
        soundBtnText.text = string.Format("SOUND EFFECTS: <color=" + color +">{0}</color>", IsMute ? "OFF" : "ON");
    }
    public void EnableVibro()
    {
        VibrateManager.Instance.SetDisable(out bool canVibrate);
        string color = canVibrate ? "green" : "red";
        vibroBtnText.text = string.Format("VIBRATION: <color=" + color + ">{0}</color>", canVibrate ? "ON" : "OFF");
    }
